...
typedef struct s_Interp {
	...
	size_t n_vars;
	size_t cap_vars;
	/* Lookups stop here, so a called function cannot see its caller's locals.
	 * Without it the flat binding stack would give dynamic scoping. */
	size_t frame_base;
...
static void envDefine(Interp *in, const char *name, size_t len, Object val) {
	if (in->n_vars == in->cap_vars) {
		size_t new_cap = (in->cap_vars == 0) ? 32 : in->cap_vars * 2;
		in->vars = pGrowAlloc(in->vars, in->cap_vars * sizeof(Binding),
		                      new_cap * sizeof(Binding), in->pool);
		in->cap_vars = new_cap;
	}
	in->vars[in->n_vars].name = name;
	in->vars[in->n_vars].len = len;
	in->vars[in->n_vars].val = val;
	in->n_vars++;
}

/* Backwards, so an inner declaration shadows an outer one. */
static Object *envLookup(Interp *in, const char *name, size_t len) {
	size_t i = in->n_vars;
	while (i > in->frame_base) {
		i--;
		if (in->vars[i].len == len && 0 == memcmp(in->vars[i].name, name, len)) {
			return &in->vars[i].val;
		}
	}
	return NULL;
}

static size_t envMark(Interp *in) {
	return in->n_vars;
}

static void envRelease(Interp *in, size_t mark) {
	in->n_vars = mark;
}
...
static void defineFunction(Interp *in, const char *name, size_t len,
	...
	if (in->n_fns == in->cap_fns) {
		...
		in->fns = pGrowAlloc(in->fns, in->cap_fns * sizeof(FnEntry),
		                     new_cap * sizeof(FnEntry), in->pool);
		in->cap_fns = new_cap;
	}
...
static Object stringFromToken(Interp *in, const Token *t) {
	...
	return objStr(objInternChars(in->pool, t->start + 1, t->len - 2));
}
...
static Flow applyBinary(Interp *in, const SyntaxNode *at, SYNTAX_TYPE op,
	...
	if (op == STX_SUM && lhs.type == STR_TYPE && rhs.type == STR_TYPE) {
		...
		char *joined = palloc(in->pool, la + lb + 1);
...
static Flow callUserFunction(Interp *in, const SyntaxNode *fn,
	...
	if (n_args > 0) {
		args = palloc(in->pool, n_args * sizeof(Object));
	...
	for (size_t i = 0; i < n_params; i++) {
		...
		if (!pname) {
			in->frame_base = saved_frame;
			envRelease(in, mark);
			in->depth--;
			return rtError(in, fn, "malformed parameter list");
		}
		envDefine(in, pname->start, pname->len, args[i]);
	}
...
static Flow evalStep(Interp *in, const SyntaxNode *node, int delta, Object *out) {
	...
	slot = envLookup(in, name->start, name->len);
	if (!slot) {
...
static Flow execDeclare(Interp *in, const SyntaxNode *node) {
	...
	envDefine(in, name->start, name->len, objNil());
	return FLOW_NORMAL;
}
...
static Flow execInit(Interp *in, const SyntaxNode *node) {
	...
	envDefine(in, name->start, name->len, value);
	return FLOW_NORMAL;
}
